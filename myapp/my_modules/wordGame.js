function WordGame(){
    var GAME_RESET_TIME = 1000 * 60 * 5; // 5 minutes
    ...
    var resetGameInactive = function() {
        m_message = "Nobody got "+m_answer+", game reset.";
        resetGame();
    }

    var getNewAnswerWord = function()
    {
        var answerIndex = Math.floor(Math.random() * possibleWords.length);
        while( !isValidGuess(possibleWords[answerIndex]) )
        {
    ...
    var getWordScore = function(word) {
        var score = 0;
        for (var letter in word) {
            if (numOccurences(m_answer, word[letter]) == 1) {
                score += 1;
            }
        }
        return score;
    ...
    this.makeGuess = function(guess, user) {
        if (isValidGuess(guess)) {
            if (m_answer == guess) {
                ...
                m_guesses.push({word: guess, num: getWordScore(guess)});
                clearInterval(m_resetInterval);
    ...
    this.getMessage = function() {
        return m_message + " Time Left: " + getTimeLeft();
    }
};
